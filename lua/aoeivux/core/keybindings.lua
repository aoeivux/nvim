...
M.maplsp = function(client, buffer)
  ...
  vim.api.nvim_buf_set_keymap(buffer, "n", "[d", "<cmd>lua vim.diagnostic.goto_prev()<CR>", opt)
  vim.api.nvim_buf_set_keymap(buffer, "n", "]d", "<cmd>lua vim.diagnostic.goto_next()<CR>", opt)
  ...
  keymap("n", "<leader>fm", function()
    ...
    vim.lsp.buf.format({
      bufnr = bfn,
      filter = function(c)
        return require("aoeivux.lsp.utils").filter_format_lsp_client(c, bfn)
      end,
    })
  end, bufopts)
  vim.api.nvim_buf_set_keymap(
